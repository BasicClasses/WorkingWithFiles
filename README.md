...
```
- **file** — указатель на указатель на файл, который получает указатель на открытый файл. 
- **path** — имя файла, который необходимо открыть. 
- **mode** — тип разрешённого доступа. 
 
Некоторые режим:

- **«r»** — открывает для чтения. Если файл не существует или не удаётся найти, вызов завершается ошибкой. 
- **«w»** — открывает пустой файл для записи. Если указанный файл существует, его содержимое удаляется. 
- **«a»** — открывается для записи в конце файла (добавление) без удаления маркера в конце файла (EOF) перед записью новых данных. Создаёт файл, если он не существует. 
- **«r+»** — позволяет для чтения и записи в существующий файл. 
- **«w+»** — позволяет для чтения и записи в новый файл. Если файл существует, его содержимое удаляется. 
- **«a+»** — позволяет для чтения и добавления в конец существующего файла.
- **b** — бинарный режим 
- **t** — текстовый режим
...
```
 errno_t fprintf_s(FILE* file, const char* format, ...);
```
